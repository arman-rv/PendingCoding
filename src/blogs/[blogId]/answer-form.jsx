...
import { replyComment } from "../../core/services/api/get-blogs";
...
export const AnswerForm = ({ comment, setIsAnswering, updateFn, user }) => {
  const form = useForm({
    defaultValues: {
      subject: "",
      message: "",
  ...
  const onSubmit = async (values) => {
    try {
      const Obj = {
        ...
        userIpAddress: `${user.userImage?.[0].userProfileId}`,
        title: values.subject,
        describe: values.message,
        userId: user.userImage?.[0].id,
